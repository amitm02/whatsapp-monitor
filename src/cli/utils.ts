...
export async function createClient(options: { verbose?: boolean } = {}): Promise<{ client: WhatsAppMonitor; config: MonitorConfig }> {
  const config = await loadConfig()
  const client = new WhatsAppMonitor(config, { verbose: options.verbose })
